...
func (h *Hub) Handle(client *Client, msg Message) {
	...
	case "join-room":
		...
		h.Join(client, msg.RoomID, msg.UserID, payload.DisplayName, payload.Password)
	case "ping":
...
func (h *Hub) Join(client *Client, roomID string, requestedUserID string, displayName string, password string) {
	if roomID == "" {
	...
	if !ok {
		...
		h.rooms[roomID] = r
	} else if !r.PasswordMatches(password) {
		h.mu.Unlock()
		client.RoomID = ""
		client.UserID = ""
		client.DisplayName = ""
		client.send(Message{Type: "error", Payload: rawError("room password is incorrect")})
		return
	}
	...
	if previous := h.clients[roomID][client.UserID]; previous != nil && previous != client {
		previous.RoomID = ""
		previous.UserID = ""
